...
export function Hero() {
  ...
  // GSAP: subtle scroll parallax on the hero image only — one meaningful effect.
  useEffect(() => {
  ...
  return (
    <section className="relative overflow-hidden bg-hero-gradient text-primary-foreground">
      ...
      <div className="container-page grid items-center gap-12 py-16 md:py-24 lg:grid-cols-2 lg:gap-16 lg:py-28">
        ...
        >
          <span className="eyebrow text-accent">Bandra West · Est. 2016</span>
          <h1 className="mt-5 font-display text-4xl leading-[1.05] font-semibold sm:text-5xl lg:text-6xl">
            Coffee worth
            <span className="block text-gradient-amber">slowing down for.</span>
          </h1>
          ...
          <dl className="mt-12 grid max-w-md grid-cols-3 gap-6 border-t border-primary-foreground/15 pt-7">
            {[
              ...
              { value: "25 min", label: "Avg. delivery", icon: Clock },
              { value: "60+", label: "Menu items", icon: null },
            ].map((stat) => (
              <div key={stat.label}>
                <dt className="flex items-center gap-1 font-display text-2xl font-semibold">
                  {stat.value}
                  {stat.icon ? <stat.icon className="size-4 fill-accent text-accent" /> : null}
                </dt>
                <dd className="mt-1 text-xs text-primary-foreground/60">{stat.label}</dd>
              </div>
            ))}
          </dl>
        </motion.div>
        ...
        >
          <div ref={imageRef} className="relative overflow-hidden rounded-[2rem] shadow-lift">
            <img
              src={heroImage}
              alt="Freshly poured latte with rosetta art on a dark wooden cafe counter"
              width={1408}
              height={1200}
            ...
            <div className="glass-dark absolute bottom-5 left-5 right-5 rounded-2xl px-5 py-4">
              <p className="text-xs uppercase tracking-[0.25em] text-accent">Today's roast</p>
              <p className="mt-1.5 font-display text-lg">Ethiopian Yirgacheffe · jasmine & stone fruit</p>
            </div>
          </div>
        </motion.div>
      </div>
