...
function AdminCategories() {
  ...
  const refresh = () => { void queryClient.invalidateQueries({ queryKey: ["admin-categories"] }); void queryClient.invalidateQueries({ queryKey: ["categories"] }); void queryClient.invalidateQueries({ queryKey: ["admin-menu"] }); };
  const createMutation = useMutation({ mutationFn: () => adminService.createCategory(form), onSuccess: () => { refresh(); notify("Category created", { variant: "success" }); setCreating(false); setForm(emptyForm); }, onError: () => notify("Couldn't create category", { variant: "error" }) });
  ...
  const filteredCategories = useMemo(() => {
    ...
    return categories.filter((category) => {
      const matchesSearch = !query || category.name.toLowerCase().includes(query) || category.slug.toLowerCase().includes(query) || category.description.toLowerCase().includes(query);
      const matchesFilter = categoryFilter === "all" || category._id === categoryFilter;
