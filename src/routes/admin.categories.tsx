...
function AdminCategories() {
  ...
  const createMutation = useMutation({
    mutationFn: () => adminService.createCategory(form),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ["admin-categories"] });
      notify("Category created", { variant: "success" });
      setCreating(false);
    },
    onError: () => notify("Couldn't create category", { variant: "error" }),
  });

  const updateMutation = useMutation({
    mutationFn: (id: string) => adminService.updateCategory(id, form),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ["admin-categories"] });
      notify("Category updated", { variant: "success" });
      setEditing(null);
    },
    onError: () => notify("Couldn't update category", { variant: "error" }),
  });

  const toggleMutation = useMutation({
    mutationFn: (category: Category) => adminService.updateCategory(category._id, { active: !category.active }),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ["admin-categories"] });
    },
    onError: () => notify("Couldn't update category", { variant: "error" }),
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ["admin-categories"] });
      notify("Category deleted", { variant: "success" });
      setPendingDelete(null);
    },
    onError: () => notify("Couldn't delete category", { variant: "error" }),
  });
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 text-3xl font-semibold tracking-tight text-foreground">Categories</h1>
        </div>
      ...
      {categoriesQuery.isLoading ? (
        ...
        <EmptyState title="No categories yet" description="Create your first category to organise the menu." />
      ) : (
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-border text-xs uppercase tracking-wider text-muted-foreground">
                ...
                <th className="hidden px-5 py-3 font-semibold sm:table-cell">Description</th>
                ...
                <tr key={category._id} className="border-b border-border last:border-0">
                  ...
                  <td className="px-5 py-3">
                    <button
                      ...
                      className={`relative h-5 w-9 rounded-full transition-colors ${category.active ? "bg-primary" : "bg-muted"}`}
                    >
                      <span
                        className={`absolute top-0.5 size-4 rounded-full bg-card shadow-soft transition-transform ${category.active ? "translate-x-4" : "translate-x-0.5"}`}
                      />
                    </button>
                  </td>
                  <td className="px-5 py-3">
                    <div className="flex justify-end gap-2">
                      ...
                      <Button variant="destructive" size="icon" onClick={() => setPendingDelete(category)}>
        ...
        footer={
          <>
            ...
            <Button
              variant="accent"
              loading={isSaving}
              onClick={() => (editing ? updateMutation.mutate(editing._id) : createMutation.mutate())}
            >
      ...
      >
        <div className="space-y-4">
          ...
          <Field id="cat-image" label="Image URL">
            <TextInput
              id="cat-image"
              value={form.image}
