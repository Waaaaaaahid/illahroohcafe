...
export const Route = createFileRoute("/admin/categories")({
  head: () => ({ meta: [{ title: "Categories — Ilarooh Admin" }] }),
...
function AdminCategories() {
  ...
  const createMutation = useMutation({ mutationFn: () => adminService.createCategory(form), onSuccess: () => { void queryClient.invalidateQueries({ queryKey: ["admin-categories"] }); notify("Category created", { variant: "success" }); setCreating(false); }, onError: () => notify("Couldn't create category", { variant: "error" }) });
  const updateMutation = useMutation({ mutationFn: (id: string) => adminService.updateCategory(id, form), onSuccess: () => { void queryClient.invalidateQueries({ queryKey: ["admin-categories"] }); notify("Category updated", { variant: "success" }); setEditing(null); }, onError: () => notify("Couldn't update category", { variant: "error" }) });
  const toggleMutation = useMutation({ mutationFn: (category: Category) => adminService.updateCategory(category._id, { active: !category.active }), onSuccess: () => { void queryClient.invalidateQueries({ queryKey: ["admin-categories"] }); }, onError: () => notify("Couldn't update category", { variant: "error" }) });
  const deleteMutation = useMutation({ mutationFn: (id: string) => adminService.deleteCategory(id), onSuccess: () => { void queryClient.invalidateQueries({ queryKey: ["admin-categories"] }); notify("Category deleted", { variant: "success" }); setPendingDelete(null); }, onError: () => notify("Couldn't delete category", { variant: "error" }) });
