...
const createPaymentOrder = asyncHandler(async (req, res) => {
  ...
  if (!isRazorpayConfigured()) return error(res, 503, 'Razorpay is not configured. Add RAZORPAY_KEY_ID and RAZORPAY_KEY_SECRET to enable online payments.');

  const { orderId } = req.body || {};
  const order = await Order.findById(orderId);
  if (!order) return error(res, 404, 'Order not found');
  if (String(order.user) !== String(user._id) && user.role !== 'admin') return error(res, 403, 'Not authorized for this order');
  if (order.paymentMethod !== 'online') return error(res, 400, 'Order is not configured for online payment');
  if (order.paymentStatus === 'paid') return error(res, 400, 'Order is already paid');

  const orderAmount = Number(order.totalAmount);
  if (!Number.isFinite(orderAmount) || orderAmount <= 0) return error(res, 400, 'Invalid order amount');

  let razorpayOrder;
  try {
    razorpayOrder = await createRazorpayOrder({ amount: orderAmount, currency: order.currency || 'INR', receipt: `order_${order.code || order._id}` });
  } catch (err) {
  ...
  const payment = await Payment.findOneAndUpdate(
    { order: order._id },
    { order: order._id, user: order.user, razorpayOrderId: razorpayOrder.id, amount: razorpayOrder.amount / 100, currency: razorpayOrder.currency || 'INR', status: 'created', paymentMethod: 'online' },
    { upsert: true, new: true, setDefaultsOnInsert: true }
  );

  return success(res, 200, { razorpayKeyId: process.env.RAZORPAY_KEY_ID, razorpayOrderId: payment.razorpayOrderId, amount: payment.amount, currency: payment.currency, paymentId: payment._id }, 'Payment order created');
});
...
const verifyPayment = asyncHandler(async (req, res) => {
  ...
  if (String(payment.user) !== String(user._id) && user.role !== 'admin') return error(res, 403, 'Not authorized for this payment');
...
const handleWebhook = asyncHandler(async (req, res) => {
  if (!process.env.RAZORPAY_WEBHOOK_SECRET) return res.status(503).json({ success: false, message: 'Razorpay webhook secret is not configured' });
  ...
  let payload;
  try { payload = JSON.parse(rawBody.toString('utf8')); } catch { return res.status(400).json({ success: false, message: 'Invalid webhook payload' }); }
...
const getPayments = asyncHandler(async (_req, res) => {
  const payments = await Payment.find().populate('order', 'code').populate('user', 'name email').sort('-createdAt').limit(100);
