...
const updateProfile = asyncHandler(async (req, res) => {
  const user = await User.findById(req.user._id).select('+password');
  if (!user) return error(res, 404, 'User not found');
  const { name, email, phone, password, savedAddresses } = req.body;
  ...
  if (Array.isArray(savedAddresses)) {
    user.savedAddresses = savedAddresses.slice(0, 10).map((entry) => ({
      _id: entry._id,
      label: String(entry.label ?? 'Home').trim().slice(0, 40) || 'Home',
      address: String(entry.address ?? '').trim().slice(0, 500),
    })).filter((entry) => entry.address.length >= 8);
  }
  if (password !== undefined) {
    if (String(password).length < 6) return error(res, 400, 'Password must be at least 6 characters');
...
const deleteUser = asyncHandler(async (req, res) => {
  if (String(req.params.id) === String(req.user._id)) return error(res, 400, 'You cannot delete your own account');
...
const updateUserRole = asyncHandler(async (req, res) => {
  ...
  if (String(req.params.id) === String(req.user._id)) return error(res, 400, 'You cannot change your own role');
