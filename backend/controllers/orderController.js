...
const createOrder = asyncHandler(async (req, res) => {
  ...
  for (const item of items) {
    ...
    if (!menuItem || !Number.isInteger(quantity) || quantity < 1) {
      return error(res, 400, 'One or more order items are invalid or unavailable');
    }
  ...
  if (
    ...
    clientSubtotal !== Number(computedSubtotal.toFixed(2))
  ) {
    return error(res, 400, 'Order totals are invalid');
  }

  const computedTotal = Number(
    (computedSubtotal + taxAmount + deliveryAmount).toFixed(2),
  );
...
const subscribeToOrder = asyncHandler(async (req, res) => {
  ...
  const cleanup = () => {
    ...
    if (subscribers) {
      subscribers.delete(res);
      if (subscribers.size === 0) orderSubscribers.delete(orderId);
    }
  };
